...
namespace MultiThreadedDownloaderLib
{
	public sealed class MultiThreadedDownloader : IDisposable
	{
		...
		public async Task<int> Download(bool accurateMode, int bufferSize = 0)
		{
			...
			LastErrorCode = GetUrlResponseHeaders(Url, Headers, out NameValueCollection responseHeaders, out string headersErrorMessage);
			...
			var tasks = chunkRanges.Select((range, taskId) => Task.Run(() =>
			{
				...
				downloader.Connected += (object sender, string url, long contentLength, int errCode) =>
				{
					...
					CallProgressUpdaterFunc(d, 0L, taskId, state);
				...
				downloader.WorkProgress += (object sender, long transferred, long contentLen, int tryNumber, int maxTryCount) =>
				{
					...
					{
						FileDownloader d = sender as FileDownloader;
						CallProgressUpdaterFunc(d, transferred, taskId, DownloadableContentChunkState.Downloading);
				...
				downloader.WorkFinished += (object sender, long transferred, long contentLen, int tryNumber, int maxTryCount, int errCode) =>
				{
					...
					if (errCode != 200 && errCode != 206 && !isExceptionRaised && !_isCanceled)
					{
						lock (downloaders)
						{
							if (!isOutOfTries)
							{
								isOutOfTries = !isInfiniteRetries && taskTryNumber + 1 > TryCountPerThread;
								if (isOutOfTries)
								{
					...
					try
					{
						...
						lock (downloaders)
						{
							...
							else
							{
								...
								{
									restartMessage += $" / {TryCountPerThread}";
								}
